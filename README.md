...
- **`web_search`** — Multi-engine web search. Uses TinyFish when `TINYFISH_API_KEY` is configured → Brave → Exa MCP hosted search (free tier first) → Exa Search API when `EXA_API_KEY` is configured → DuckDuckGo → Bing when corresponding paths are available, with Google scraping available as an opt-in last resort. Responses are text-first and render the full result set in `content`.
...
Returns an array of `{ engine, title, url, snippet }` where `engine` is one of `"TinyFish"`, `"Brave"`, `"Exa"`, `"DuckDuckGo"`, `"Bing"`, or `"Google"`.
...
The fallback chain is TinyFish → Brave → Exa MCP hosted search → Exa Search API → DuckDuckGo → Bing, with Google scraping appended only when `OPENSEARCH_ENABLE_GOOGLE_SCRAPE=true`. TinyFish is skipped when `TINYFISH_API_KEY` is not configured. The hosted Exa MCP path is tried before raw Exa API calls so the server can use Exa's free hosted tier before consuming a configured `EXA_API_KEY`. Set `OPENSEARCH_ENABLE_EXA_MCP=false` to skip the hosted Exa MCP path entirely. If configured TinyFish, Brave, or raw Exa credentials are rejected, the server continues down the fallback chain instead of aborting the search.


Returns a compact text rendering of the full result set in `content`, with each result rendered in `Title` / `URL` / `Highlights` / `Source` form. `engine` is one of `"TinyFish"`, `"Brave"`, `"Exa"`, `"DuckDuckGo"`, `"Bing"`, or `"Google"`.
